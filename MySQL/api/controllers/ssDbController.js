...
exports.addCourse = function(req, res) {
     ...
     Course.addCourse(newCourse,function(err,result) {
       if (err) {
           res.send(err);
       } else {
           res.json(result);
...
/* getCourse -- Process API request to get all data associated with a given course.
   We do this by callin on the corresponding getCourse method in the model. 
*/
exports.getCourse = function(req, res) {
   Course.getCourse(req.params.courseId, function(err, result) {
       if (err) {
         res.send(err);
       } else {
         res.json(result);
...
/* updateCourseHole -- Process API request to update hole data.
   We dot his by calling on the corresponding updateCourseHole method in the model.
*/
exports.updateCourse = function(req, res) {
    Course.updateCourse(req.params.courseId, new Hole(req.body), function(err,result) {
      if (err) {
          res.send(err);
      } else {
          res.json(result);
